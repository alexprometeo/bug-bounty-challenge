...
const getInitials = (user: User) => {
  ...
  // This way it's too messy for me
...
// I was considering implementing this function but I don't think it will be much more efficient
// so an example will be const first = upperCaseThis(user?.firstName?.[0]) || "";
const upperCaseThis = function (textToUpperCase: string) {
  if (!textToUpperCase) return "";
  const upperCasedText = textToUpperCase.toLocaleUpperCase();

  return upperCasedText;
};

const stringAvatar = (user: User) => {
  // Just in case we don't have any user, show something at least
  if (!user || (!user.firstName && !user.lastName)) {
    return {
      sx: { bgcolor: "rgb(100,100,100)", cursor: "pointer" },
      children: "?",
    };
  }
  ...
  // In case the firstName is undefined or null
